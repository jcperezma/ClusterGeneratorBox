    ...
    do i=1,nbr_bins
        ...
        do ll=1,bin_heights(i)
            ...
            do while (flag .eqv. .false.)
                ...
                theta=r*max_theta
                !print *, "flag ", flag
                ! To avoid fibers being aligned entirely in Z. In the precompression the will break.
                if ( (abs(theta - 3.1416*0.5)) < z_orientation ) then
                    flag=.false.
                    CYCLE
                end if
                ...
                !Check that fiber is within  the box
                do j=1, 3
                ...
                ! if it is outside generate a new fiber
                IF (.not. flag) then
                    !print *, " ini is out"
                    CYCLE
                end if
                ...
                !print *, "coords ini ", coords_ini(kk,:)
                !print *, "coords end ", coords_end(kk,:)       
                ! print *, box_size/2.0
                ! print *, "flag ", flag
                ! if it is outside generate a new fiber
                IF (.not. flag) then
                ...
                ! Check if it overlaps with the other fibers
                if (kk.ne.1) then
                    do k=1, kk-1
                        fibs_collide=.false.
                        ...
                        ! Compare first their bounding boxes, It is a cheaper
                        if (are_boxes_intersect(ra, ra_end,rb, rb_end, fiber_diameter)) then
                        
                        ! If their bounding boxes intersect find if the fibers overlap
                        call collide2   (ra,&
                        ra_end,&
                        rb,&
                        rb_end,&
